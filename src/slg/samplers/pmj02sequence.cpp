...
void PMJ02Sequence::RequestSamples(const u_int size) {
	...
	for (u_int i = 0; i < tablesToGenerate; i++) {
		SLG_LOG("Generating " << num_samples << " samples for " << 2*i << " and " << 2*i+1 << " dimensions on " << i << " table");
		...
		SLG_LOG("Generated " << num_samples << " samples for " << 2*i << " and " << 2*i+1 << " dimensions on " << i << " table");
	}
